...
@with_session
def collect_week_tacks(
    period_start: str, period_end: str, session: Session = None
) -> list:
    platform_id = get_platform_id("BP")

    tracks = (
        session.query(PlatformTrack)
        .options(joinedload(PlatformTrack.track))
        .filter(
            PlatformTrack.platform_id == platform_id,
            PlatformTrack.release_date.between(period_start, period_end),
        )
        .all()
    )

    return [
        {"isrc": track.track.isrc, "id": track.platform_track_id} for track in tracks
    ]
...
def get_or_create_track_bp(bp_track: dict, artists_ids, session=None) -> int:
    artists = [
        session.query(Artist).filter_by(id=artist_id).first()
    ...
    new_track = Track(
        title=bp_track["title"],
        isrc=bp_track["isrc"],
        artists=artists,
        style_id=bp_track["style_id"],
    )
...
def connect_track_sp_to_bp(track_bp_id, sp_track, session=None):
    ...
    old_track_bp = (
        session.query(PlatformTrack)
        .filter_by(platform_id=bp_platform_id, platform_track_id=track_bp_id)
        .first()
    )
    if not old_track_bp:
    ...
    old_track_sp = (
        session.query(PlatformTrack)
        .filter_by(platform_id=sp_platform_id, platform_track_id=track_sp_id)
        .first()
    )
    if old_track_sp:
...
def reg_bp_track(track: dict, session: Session = None) -> int:
    logger.info(f"Registering track :: BP ID : {track['id']} :: Start")
    artists = [
        {"bp_id": str(artist["id"]), "name": artist["name"]}
        for artist in track["artists"]
    ]
    artists_ids = [
        get_or_create_artist_bp(artist, session=session) for artist in artists
    ]
    track_data = {
        "bp_id": str(track["id"]),
        "title": track["name"],
    ...
    logger.info(f"Registering track :: BP ID : {track['id']} :: Done")
    return track_id
