...
from src.mongo_adapter import collect_bp_releases, save_data_mongo, collect_releases_tracks
...
if __name__ == "__main__":
    ...
    #     logger.info(f"Handle release from bp : {idx}/{len(release_ids)} :: Start")
    #     for tracks in handle_one_release(release_id, bp_url, bp_token):
    #         save_data_mongo(tracks, "bp_tracks")
    #     logger.info(f"Handle release from bp : {idx}/{len(release_ids)} :: Done")

    tracks = collect_releases_tracks(release_ids)
    print(tracks)
    print(len(tracks))
